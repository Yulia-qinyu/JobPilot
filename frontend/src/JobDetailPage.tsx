...
export default function JobDetailPage() {
  ...
  return <main className="workspace-shell job-detail-page">
    ...
    {tab === "overview" && <div className="detail-layout"><div className="detail-main">
      <section className="quick-overview"><span className="card-kicker">岗位概览</span><div className="one-line-summary"><span>这个岗位主要做什么</span><h2>{structured.role_summary || "该岗位暂未生成简要概述。"}</h2></div><div className="quick-columns"><div><h3>关键要求</h3>{structured.key_requirements.length ? <ol>{structured.key_requirements.map((item, index) => <li key={`${item.title}-${index}`}><span className={`priority priority-${item.priority}`}>{index + 1}</span><div><strong>{item.title}</strong><p>{item.explanation}</p></div></li>)}</ol> : <p className="muted">该 JD 中未提取到明确重点要求。</p>}</div><div><h3>相关知识与领域</h3><div className="topic-tags">{structured.knowledge_topics.map((topic) => <span key={topic}>{topic}</span>)}{!structured.knowledge_topics.length && <p className="muted">该 JD 中未提取到明确知识主题。</p>}</div></div></div></section>
