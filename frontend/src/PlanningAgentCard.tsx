...
export default function PlanningAgentCard({ planning, loading, error, onGenerate, onAddToPlan }: {
  ...
  return <section className="planning-agent-card" aria-label="今日规划助手">
    <div className="planning-agent-heading">
      <div><span className="agent-sparkle">✦</span><div><span className="eyebrow">On-demand planning</span><h2>{snapshot ? "今天可以优先做这些" : "今天需要一点小助攻吗？"}</h2></div></div>
    ...
    {snapshot && <>
      ...
      <div className="advice-list">{snapshot.items.map((item) => <article className="advice-item" key={item.id}>
        <div className="advice-priority" data-priority={item.priority}>{PRIORITY_LABELS[item.priority]}</div>
        <div className="advice-copy"><h3>{item.title}</h3><p>{item.reason}</p></div>
