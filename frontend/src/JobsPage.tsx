...
import NudgeModule from "./NudgeModule";
...
export default function JobsPage() {
  ...
  return <main className="workspace-shell jobs-page">
    <header className="workspace-heading compact"><div><span className="eyebrow">求职进度</span><h1>我的岗位</h1><p>统一管理准备投递和已经投递的岗位。</p></div><div className="heading-actions"><button className="secondary-button" onClick={() => setManaging((value) => !value)}>管理状态</button><Link className="primary-link" to="/jobs/new">+ 手动添加岗位</Link></div></header>
    <NudgeModule />
    {managing && <section className="status-manager"><div><h2>管理状态</h2><p>默认状态可重命名和排序；自定义状态在没有岗位使用时可以删除。</p></div><div className="status-definition-list">{statuses.map((item) => <div key={item.id}><span>{item.label}</span><small>{item.is_system_default ? "默认状态" : "自定义状态"}</small><button onClick={() => void renameStatus(item)}>重命名</button>{!item.is_system_default && <button className="danger-text-button" onClick={() => void removeStatus(item)}>删除</button>}</div>)}</div><div className="inline-add"><input value={newStatus} onChange={(event) => setNewStatus(event.target.value)} placeholder="例如：一面" /><button onClick={() => void addStatus()}>新增状态</button></div></section>}
