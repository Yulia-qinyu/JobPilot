...
Identical to PROMPT_A_CONTROL except for the inserted **ADJUDICATION RULES (rubric-aligned v1)** block (5 frozen adjudication rules), placed immediately before the existing `IMPORTANCE:` section. No Ground-Truth examples, no benchmark cases, no model-specific hints, no new taxonomy / scoring / eligibility / knowledge / preference logic. The production prompt file is NOT modified; this text is used only by the eval runner.
